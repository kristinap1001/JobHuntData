...
def makeDiagram(jobDf, incSources=True):
  ...
  if incSources == False:
    df = pd.DataFrame(secondPath, columns=['Source','Target','Count'])
    df['Count'] = df['Count'].astype(int)
    df['Source'] = df['Source'].replace(df['Source'].unique().tolist(), 'Applied')
    df['Count'] = df['Count'].groupby(df['Target']).transform('sum')
    df.drop_duplicates(inplace=True)
    secondPath = df.to_numpy()
  ...
  if incSources == True:
    ...
    sankeyTable = np.vstack((secondPath,thirdPath,fourthPath))
  ...
  if incSources == True:
    ...
    colors = colors + ['rgba(140, 140, 140, 1)'] * (len(jobSources))
  ...
  if incSources == True:
    fig.write_html("SankeyDiagram.html")
      ...
      fig.write_html("SankeyDiagramNoSources.html")
  fig.show()

makeDiagram(jobDf,incSources=True)
makeDiagram(jobDf,incSources=False)